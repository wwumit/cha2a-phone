...
- 插件默认对接 **`https://compliancehub.cn` 演示服务端**（CHA2A 参考实现，容量有限；生产使用请自托管服务端并配置 `AGENT_PHONE_REGISTRY`）。
...
- **最小权限（宿主配置）**：外发/开户/收消息类工具默认不启用（optional）——若宿主以 `tools.allow` 配置本插件，只放行当前任务需要的具体工具，不要 `tools: {}`（等同 full）。
- **手机 UI 身份**：`assets/phone.html` 不内置默认身份/号码——打开/内嵌前必须配置 `?agentDid=&numA=&numB=`（或 `__DSH_PHONE_CONFIG__`）。未配置（无 URL/全局配置/本地已存身份）时只显示开户引导、**不发起任何网络请求**；本地保存过身份（`cha2a-phone-identity`）视为已配置并恢复会话。不要用演示/他人身份打开。
