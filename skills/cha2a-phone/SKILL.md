...
- **手机 UI 身份**：`assets/phone.html` 不内置默认身份/号码——打开/内嵌前必须配置 `?agentDid=&numA=&numB=`（或 `__DSH_PHONE_CONFIG__`）。未配置（无 URL/全局配置/本地已存身份）时只显示开户引导、**不发起任何网络请求**；本地保存过身份（`cha2a-phone-identity`）视为已配置并恢复会话。不要使用他人的身份/号码打开。
